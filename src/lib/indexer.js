...
export async function buildIndex(config, options = {}) {
  ...
  try {
    ...
    const vectorsByChunk = await embedChangedChunks(store, chunks, embedder);
...
async function embedChangedChunks(store, chunks, embedder) {
  ...
  if (changed.length) {
    const embedded = await embedder.embed(changed, 'passage');
    for (let i = 0; i < embedded.length; i += 1) {
      vectors[changedIndexes[i]] = [embedded[i]];
    }
  }
